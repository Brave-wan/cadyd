...
public class RouteActivity extends BaseActivity implements OnMapClickListener,
    ...
    private String mCurrentCityCode = "023";
    ...
    protected void onCreate(Bundle bundle) {
        ...
        mCurrentCityCode = city.areacode;
        mStartPoint = new LatLonPoint(Double.valueOf(city.latitude), Double.valueOf(city.longitude));
        mEndPoint = new LatLonPoint(getIntent().getDoubleExtra("endLat", 0.0), getIntent().getDoubleExtra("endLong", 0.0));
        setView(R.layout.route_activity, "导航", true);
    ...
    private void setfromandtoMarker() {
        ...
        Marker mk = aMap.addMarker(new MarkerOptions()
                .position(AMapUtil.convertToLatLng(mEndPoint))
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.end)));
        //mk.setTitle("商家点名");
        //mk.showInfoWindow();
        //设置中心点和缩放比例
        aMap.moveCamera(CameraUpdateFactory.changeLatLng(new LatLng(mEndPoint.getLatitude(), mEndPoint.getLongitude())));
    ...
    public void onBusClick(View view) {
        searchRouteResult(ROUTE_TYPE_BUS, RouteSearch.BusLeaseWalk);
    ...
    public void searchRouteResult(int routeType, int mode) {
        if (mStartPoint == null) {
            ToastView.show(mContext, "定位中，稍后再试...");
            return;
        }
        if (mEndPoint == null) {
            ToastView.show(mContext, "终点未设置");
        }
        showProgressDialog();
        ...
        if (errorCode == 1000) {
            if (result != null && result.getPaths() != null) {
                if (result.getPaths().size() > 0) {
                    ...
                    ToastView.show(mContext, R.string.no_result);
                }
            } else {
                ToastView.show(mContext, R.string.no_result);
            }
        } else {
            ToastView.show(this.getApplicationContext(), R.string.no_result);
        }
    }
