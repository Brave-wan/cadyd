...
public class ZImageCycleView extends LinearLayout {
     ...
     * @param context
     */
    public ZImageCycleView(Context context) {
        super(context);
    }
     ...
     * @param attrs
     */
    public ZImageCycleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
    ...
    private String getPath(Object object, String imageName) {
        if (object == null || imageName == null) {
            return "请出入正确的对象及参数名";
        }
        String content = null;
        try {
            Field f = object.getClass().getDeclaredField(imageName);
            f.setAccessible(true);
            content = f.get(object).toString();
     ...
     * 装填图片数据
     */
    public void setImageResources(List objects, String imageName, int resid, ImageCycleViewListener imageCycleViewListener) {
        if (mGroup.getChildCount() > 0) {
            // 清除所有子视图
            mGroup.removeAllViews();
        }
        // 图片广告数量
        final int imageCount = objects.size();
        ...
        mAdvAdapter = new ImageCycleAdapter(mContext, objects, imageName, imageCycleViewListener, resid);
    ...
    private class ImageCycleAdapter extends PagerAdapter {
        ...
        private String imageName = null;//图片的名字
        ...
        public ImageCycleAdapter(Context context, List<Object> adList, String imagName, ImageCycleViewListener imageCycleViewListener, int resid) {
            mContext = context;
            mAdList = adList;
            imageName = imagName;
            this.resid = resid;
        ...
        @Override
        public int getCount() {
            return mAdList == null || mAdList.size() <= 0 ? 0 : Integer.MAX_VALUE;
        }
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            String imageUrl = getPath(mAdList.get(index), imageName);
            ...
            imageView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mImageCycleViewListener != null) {
                        mImageCycleViewListener.onImageClick(index, mAdList.get(index), v);
                    }
                }
            });
